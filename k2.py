...
def index():
    ...
    # print(online_list)
    return render_template('index.html',time_=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
...
def room_upload(id):
    ...
    # if ip_info[0] != '中国':
        ...
        # color = 'black'
    ip2='UNKNOW'
    l[id].insert(0, [name, word, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),'未登录'])
    online[id] = time.time()
...
def rl():
    ...
    for i in online.items():
        if i[1] == 0:
            ...
            online_list.append([i[0], time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(i[1]))])
    return render_template('room_list.html',online_list=online_list,l=qww)
